...
io.on('connection', socket => {
  ...
  //Event to database
  var event = new Event({ socket_id :socket.id, username:socket.username,
    room:socket.room,action:`Anonymous user with id ${socket.id} made a connection`});
  ...
  socket.on("disconnect", function(data){
    ...
    online_users.splice(online_users.indexOf({socket_id: socket.id,
      username: socket.username, room: socket.room}),1);
    socket.broadcast.emit("get users",online_users);
    console.log("Disconnected: "+online_users.length +" users left"); 
    //Event to database
    var event = new Event({socket_id :socket.id, username:socket.username,
      room:socket.room,action:`${socket.username} with id ${socket.id} disconnected!`});
    event.save(function (err, event) {
    if (err) return console.error(err);
